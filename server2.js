...
app.get('/api/market/:symbol/candles',async(req,res)=>{try{const symbol=decodeURIComponent(req.params.symbol);const interval=String(req.query.interval||'1m');const limit=Math.min(Math.max(Number(req.query.limit)||120,20),300);const m=await marketCandles(symbol,limit,interval);res.set('Cache-Control','no-store');res.json({symbol,interval,candles:m.candles,source:m.source});}catch(e){res.status(503).json({error:'Market candles temporarily unavailable'});}});
